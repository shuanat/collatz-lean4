...
/-- Paper-faithful phase data carried by one selected segment modulo `Q_t`. -/
structure QTPhaseSegmentWitness (t i j : ℕ) where
  phaseClass : ℕ
  phaseClass_lt : phaseClass < Q_t t
  phaseStart : i % Q_t t = phaseClass
  phaseEnd : j % Q_t t = phaseClass
...
lemma ap_structure_complete (k_0 : ℕ) (M_tilde_k0 : ℕ) (t : ℕ) (_ht : 3 ≤ t) :
    touch_indices_tail k_0 M_tilde_k0 t = {k : ℕ | k ≡ k_0 [MOD (Q_t t)]} := by
  rfl
